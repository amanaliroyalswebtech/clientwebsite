...
import { FaUserTie, FaWallet, FaCogs, FaUsers, FaRocket, FaArrowLeft } from 'react-icons/fa';
        ...
        <>
            <section className="py-16 bg-white mt-20 md:mt-40">
                <div className="container mx-auto text-center relative">
                    {/* Link to return home positioned to the left */}
                    <Link
                        to="/"
                        className="absolute left-4 top-40 md:left-6 md:top-2 flex items-center text-indigo-500 hover:text-indigo-700 transition-colors duration-300"
                    >
                        <FaArrowLeft className="text-2xl" />
                        {/* Hide on small screens and show on larger screens */}
                        <span className="ml-2 text-sm font-semibold hidden md:block">Return Home</span>
                    </Link>

                    <h2 className="text-4xl font-bold mb-8 animate-slideIn">Account Section</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {services.map((service, index) => (
                                ...
                                >
                                    ...
                                    <h3 className="text-xs md:text-sm font-semibold text-gray-800 mb-1 text-center break-words">{service.name}</h3>
                                </a>
                            ))}
                        </div>
                    </div>
